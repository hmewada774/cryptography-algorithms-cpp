...
{
    int result = (half ^ key) & 0xF;                                  // XOR operation and masking with 0xF to get 4-bit output
...
// CTR Encryption
string ctrEncrypt(string plaintext, int iv, int k1, int k2, int k3, int k4)
{
    string ciphertext = "";
    int counter = iv;
    ...
    {
        int encCounter = encryptBlock(counter, k1, k2, k3, k4);
        int enc = c ^ encCounter;
        ciphertext += (char)enc;
        counter++;
    }
    return ciphertext;
...
// CTR Decryption
string ctrDecrypt(string ciphertext, int iv, int k1, int k2, int k3, int k4)
{
    string plaintext = "";
    int counter = iv;
...
int main()
{
    ...
    // Encryption
    string encrypted = ctrEncrypt(text, iv, k1, k2, k3, k4);
    ...
    // Decryption
    string decrypted = ctrDecrypt(encrypted, iv, k1, k2, k3, k4);
